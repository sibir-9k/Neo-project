...
export default React.memo(function ClaimForm ({ formData }) {
  ...
  useEffect(() => {
    if (formData) {
      setForm({ title: formData.title, description: formData.description, type: formData.type.slug, status: formData.status.slug, _id: formData._id });
    }
    dataBase()
      .get("/types")
  ...
  return (
    <>
      ...
      {role.slug === "work" &&
        <Button value="Cansel" type="cansel" className="form-cansel__btn" />
      }
      {role.slug === "work" &&
        <Button
          value="Create"
          ...
          onCreatedClick={() => { onCreatedClick(form); }}
        />
      }
    </>
  );
